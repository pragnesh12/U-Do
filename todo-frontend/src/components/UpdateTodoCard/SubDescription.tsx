...
const SubDescription = ({ id, subTitle, subDone }: any) => {
  const { handleUpdateSubTodo, handleDeleteSubTodo, setTodo } =
    useContext(TodoContext);

  // Initialize the local state with subDone value received from backend
  const [isChecked, setIsChecked] = useState(subDone);

  // This effect ensures the state updates if subDone changes
  useEffect(() => {
    setIsChecked(subDone);
  }, [subDone]);

  const toggleCheck = () => {
    const updatedStatus = !isChecked;
    setIsChecked(updatedStatus); // Optimistically update the local state
    handleUpdateSubTodo(id, { subDone: updatedStatus }); // Send the update to the backend
  };
    ...
    try {
      ...
      setTodo((prevTodos: any) =>
        prevTodos.map((todo: any) => ({
          ...todo,
          subTodos: todo.subTodos?.filter((subTodo: any) => subTodo.id !== id),
        }))
      );

      // Call the delete API
      const response: any = await handleDeleteSubTodo(id);
      if (!response.success) {
